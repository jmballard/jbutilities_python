...
def check_format_df(df,digits = 2):
    ...
    if not isinstance(digits, (int)):
        raise TypeError("The input 'digits' is not of the correct format. Needs an integer.")

    if not isinstance(df, pd.DataFrame):
        raise TypeError("The input 'data' is not of the correct format. Needs a dataset.")
    
    def happy(string):
        print(emoji.emojize('\033[1; 32m :smiley: : ' + string + '\033[0 m', 
                            language='alias'))

    def unhappy(string):
        print(emoji.emojize('\033[1; 31m :x: : ' + string + '\033[0 m', 
                            language='alias'))
    
    df_summary = np.transpose(
        df.describe(
            include = 'all',
    ...
    if(any(df_summary.index != df_summary['columns_snake_case'])):
        ...
        for col in df_summary.index[df_summary.index != df_summary['columns_snake_case']]:
            unhappy(f"\t- The column {col} should have been {df_summary.loc[col,'columns_snake_case']}")
    else:
    ...
    if(any(df_summary['missing_rows_pct'] > 0)):
        ...
        for col in df_summary.index[df_summary['missing_rows_pct'] > 0]:
            unhappy(f"\t- The column {col} has {df_summary.loc[col,'missing_rows_pct']}% missing value")
    else:
    ...
    df_summary['infinite_rows_pct'] = np.round(
        pd.DataFrame(
            ((df == math.inf) |(df == -math.inf) ).sum()/df.shape[0]
            )*100,
        digits
        )
    ...
    if(any(df_summary['infinite_rows_pct'] > 0)):
        ...
        for col in df_summary.index[df_summary['infinite_rows_pct'] > 0]:
            unhappy(f"\t- The column {col} has {df_summary.loc[col,'infinite_rows_pct']}% infinite value")
    else:
    ...
    df_summary['num_levels'] = pd.DataFrame(
        df.nunique()
        )        
    
    df_summary = df_summary[['columns_snake_case', 
                             'data_type',
                             'type_cat_or_num',
                             'count',
                             'missing_rows_pct',
