...
def create_df_rd(size = 10,
    ...
    if not isinstance(size, int):
        raise TypeError("The input 'size' is not of the correct format. Needs an integer.")

    if seed is None:
        seed = (datetime.now().hour * 3600 +
    ...
    if numerics is not None:
        for key, val in numerics.items():
            ...
            if val is None:
                print(f"\tThe column '{key}' is normaly distributed with mean 0 and std 1")
                df[key] = np.random.normal(loc = 0, 
                                             scale = 1,
                                             size = size) 
            else:
                print(f"\tThe column '{key}' is normaly distributed with mean {val[0]} and std {val[1]}")
                df[key] = np.random.normal(loc = val[0],
                                             scale = val[1],
                                             size = size) 
            i += 1
    ...
    if booleans is not None:
        for key, val in booleans.items():
            np.random.seed(seed + i)
            if val is None:
                print(f"\tThe column '{key}' is binomialy distributed with proba 0.5 of having 1")
                df[key] = np.random.binomial(n = 1, 
                                             p = 0.5,
                                             size = size).astype('bool')
            else:
                print(f"\tThe column '{key}' is binomialy distributed with proba {val} of having 1")
                df[key] = np.random.binomial(n = 1,
                                             p = val,
                                             size = size).astype('bool')
            i += 1
    ...
    if categories is not None:
        for key, val in categories.items():
            ...
            if val is None:
                ...
                print(f"\tThe column '{key}' is binomialy distributed with proba {round(1/len(val),2)} for each value")
                answers = pd.DataFrame(np.random.multinomial(n = 1,
                                             pvals = [1/len(val) for i in range(len(val))],
                                             size = size))
                df[key] = val[0]
                for j in range(1,len(val)):
                    df[key] = np.where(answers[j] == 1,
                                       val[j],
                                       df[key])
